...
// 这段代码实现了一个名为 `Module` 的部分类，主要功能包括：

// 1. 定义了 SPIR-V 模块的基本属性和常量。
// 2. 提供了构造函数，用于初始化模块的版本号和对象池。
// 3. 提供了方法用于添加模块的功能、扩展、指令等元素。
// 4. 提供了方法用于生成 SPIR-V 模块的字节码。
// 5. 包含一些私有方法和字段，用于内部操作和管理对象池。
// 6. 模块的功能和指令都以列表的形式存储，方便管理和生成字节码时的顺序控制。